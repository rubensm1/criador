...
class Connexao {
    ...
    function Connexao($database = DataBaseConfig::BANCO, $host = DataBaseConfig::HOST, $port = DataBaseConfig::PORT, $user = DataBaseConfig::USER, $pass = DataBaseConfig::PASS) {
		try {
			if ($pass == '')
				$this->conexao = new PDO('mysql:host=' . $host . ';port=' . $port . ';dbname=' . $database, $user); // conecta o servidor
			else
				$this->conexao = new PDO('mysql:host=' . $host . ';port=' . $port . ';dbname=' . $database, $user, $pass); // conecta o servidor
